...
//11) set()을 호출할 때 인덱스가 유효하지 않으면 인덱스의 값을 꺼낼 때 예외를 발생시킨다.

//테스트3 - MyArrayList3
//12) 여러개의 목록을 동시에 관리할 수 있도록
//      MyArrayList의 레퍼런스 배열을 static대신 인스턴스로 전환한다.
...
//15) ArrayList 인스턴스를 생성할 때 초기 크기를 지정하지 않고 생성할 수 있도록 기본 생성자를 추가
...
//21) toArray에서 배열을 복사할 때 Arrays.copyOf() 메서드를 활용해보자.
...
public class MyArrayList21 {
  ...
  private void grow() {
    ...
    elementData = Arrays.copyOf(elementData, newCapacity); //새 배열을 만들어서 기존 값 복사
    /*
  ...
  public Object remove(int index) {
    ...
    //쓰지 않는 인스턴스의 주소를 제거하여 가비지 될 수 있게 한다.
    return old;
  }
