...
//      Node의 생성자를 추가한다.
// 7) 목록에서 특정 인덱스의 값을 제거하는 remover(int) 메서드를 정의한다.
// 8) 목록에서 특정 인덱스의 값을 바꾸는 set(int,Object) 메서드를 정의한다.
// 9) 목록의 데이터를 새 배열에 담아 리턴하는 새 배열에 담아 리턴하는 toArray()메서드에 정의한다.
...
public class MyLinkedList10 {
  ...
  public boolean add(Object e) {
    ...
    if(first == null) {
      ...
      last.next = node; // 새로 만든 노드와 연결시키기 위해 새로만든 노드 주소를 객체에 담는다.
    }

    last = node; // last가 새 노드를 가리킨다.
    size++;
  ...
  public Object remove(int index) {
    ...
    cursor.next = old.next; //삭제하려는 객체의 다음 객체
    old.next = null; //가비지가 다른 인스턴스를 가리키지 않게 한다.
  ...
  public Object set(int index, Object element) {
    ...
    cursor.value = element; //cursor의 값을 새값으로 변경
